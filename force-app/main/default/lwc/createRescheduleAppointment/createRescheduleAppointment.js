...
import relatedToId from '@salesforce/apex/RescheduleAppointmentFormController.relatedToId';//added by Aniket for Shubham's new requirement 
...
export default class AppointmentForm extends NavigationMixin(LightningElement) {
    ...
    getServiceCenter(){
        debugger;
        getCurrentServiceCenter()
        .then(result=>{
            console.log('Account Detail===>',result);
            this.serviceCenterId = result.accountId;
            this.serviceCenterName = result.accountName;
        })
        .catch(error=>{
