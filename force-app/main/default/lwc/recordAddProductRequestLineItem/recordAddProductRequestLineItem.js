...
export default class RecordAddProductRequestLineItem extends LightningElement {
    ...
    handleDeleteSelectedItem(event) {
        ...
        if (deletedItem) {
            ...
            const originalIndex = deletedItem.index - 1; 
            
            this.currentPageData = this.currentPageData.filter(item => item.Id !== itemId);
    
            
            this.currentPageData = [
                ...this.currentPageData.slice(0, originalIndex),
                {
                    ...deletedItem,
                    selected: false, 
                    index: deletedItem.index 
                },
                ...this.currentPageData.slice(originalIndex)
            ];
    
            
            this.currentPageData = this.currentPageData.map((item, idx) => ({
                ...item,
                index: idx + 1 
            }));
        }
